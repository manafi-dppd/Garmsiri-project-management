  ...
  return (
    <div className="container justify-center flex flex-wrap items-center gap-4 p-2 bg-blue-100 rounded-lg shadow-md">
      ...
      <div className="flex items-center gap-2">
        <label className="font-semibold">سال زراعی</label>
        {isNovember ? (
          <select
            className="border p-2 rounded-lg"
            onChange={(e) => handleAgriculturalYearChange(e.target.value)}
            value={agriculturalYear}
          >
            {currentJalaliYear > 0 && (
              <>
                <option value={`${currentJalaliYear - 1}-${currentJalaliYear}`}>
                  {currentJalaliYear - 1}-{currentJalaliYear}
                </option>
                <option value={`${currentJalaliYear}-${currentJalaliYear + 1}`}>
                  {currentJalaliYear}-{currentJalaliYear + 1}
                </option>
              </>
            )}
          </select>
      ...
      {/* دوره کشت */}
      <div className="flex items-center gap-2">
        <label className="font-semibold">دوره کشت</label>
        <input
          type="text"
          value={farmingSeason}
          readOnly
